...
def _get_item_key(obj):
    ...
    if isinstance(obj, Account):
        return f"account:{str(obj.index).zfill(6)}"

    if isinstance(obj, Block):
        return f"block:{obj.timestamp}.{obj.bhash}"

    if isinstance(obj, Deploy):
        return f"deploy:{obj.ts_dispatched}.{obj.dhash}"
    ...
    if isinstance(obj, RunEvent):
        return f"event:{obj.timestamp}.{obj.event}"

    if isinstance(obj, Transfer):
        return f"transfer.{obj.asset.lower()}:{obj.dhash}"
