...
def transfer(ctx: ExecutionContext, cp1: Account, cp2: Account, amount: int) -> typing.Tuple[Node, str]:
    ...
    log_event(MonitoringEventType.DEPLOY_DISPATCHED, node, message=f"TRANSFER_U512_STORED {amount} CLX from {cp1.public_key[:8]} to {cp2.public_key[:8]}", deploy_hash=deploy_hash)
