...
def transfer(ctx: ExecutionContext, cp1: Account, cp2: Account, amount: int) -> typing.Tuple[Node, str, float, int]:
    ...
    log_event(EventType.WFLOW_DEPLOY_DISPATCHED, f"{deploy_hash} :: TRANSFER_U512 {amount} CLX from {cp1.public_key[:8]} to {cp2.public_key[:8]}", node, deploy_hash=deploy_hash)
