...
def stream_events(node_id: NodeIdentifier, event_callback: typing.Callable):
    ...
    for info in client.stream_events(all=True):        
        ...
        if info.HasField("block_added"):
            ...
            event_type=EventType.MONIT_DEPLOY_REQUEUED
            block_hash=None
            deploy_hash=info.deploy_requeued.deploy.deploy_hash.hex()

        else:
            log_event(EventType.MONIT_STREAM_EVENT_TYPE_UNKNOWN, f"event skipped as type is unsupported :: node={node.address} :: event-id={info.event_id}", node)
            continue
