...
class EnvVars:
    # RabbitMQ host.
    HOST = env.get_var('MQ_BROKER_RABBIT_HOST', "localhost")

    # RabbitMQ port.
    PORT = env.get_var('MQ_BROKER_RABBIT_PORT', 5672, int)

    # RabbitMQ protocol.
    PROTOCOL = env.get_var('MQ_BROKER_RABBIT_PROTOCOL', "amqp")

    # RabbitMQ SSL client cert.
    SSL_CLIENT_CERT = env.get_var('MQ_BROKER_RABBIT_SSL_CLIENT_CERT')

    # RabbitMQ SSL client cert key.
    SSL_CLIENT_KEY = env.get_var('MQ_BROKER_RABBIT_SSL_CLIENT_KEY')

    # RabbitMQ user.
    USER = env.get_var('MQ_BROKER_RABBIT_USER', "stests-mq-user")

    # RabbitMQ user password.
    USER_PWD = env.get_var('MQ_BROKER_RABBIT_USER_PWD', "clabs")
