...
def do_phase(ctx: ExecutionContext):
    ...
    # Notify.
    log_event(EventType.WFLOW_PHASE_START, ctx.label, ctx)
...
def on_phase_end(ctx: ExecutionContext):
    ...
    # Notify.
    log_event(EventType.WFLOW_PHASE_END, ctx.label_phase_end, ctx)
...
def on_phase_error(ctx: ExecutionContext, err: str):
    ...
    # Notify.
    log_event(EventType.WFLOW_PHASE_ERROR, f"{ctx.label} : {err}", ctx)
...
def _can_start(ctx: ExecutionContext) -> bool:
    ...
    if phase is None:
        log_event(EventType.WFLOW_PHASE_ABORT, ctx.label, ctx)
        return False
