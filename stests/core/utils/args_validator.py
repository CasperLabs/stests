...
def validate_network(value):
    ...
    # Validate network type.
    gen = (i for i in NetworkType if name.startswith(i.name.lower()))
    matching_network_type = next(gen, None)
    if not matching_network_type:
        raise argparse.ArgumentError("Invalid network name")

    # Validate network index.
    validate_network_index(name[len(matching_network_type.name):])
