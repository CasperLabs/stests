...
class NodeApi_Exception(Exception):
    ...
    def __str__(self):
        ...
        """
        return u"STESTS <-> NODE API EXCEPTION : {0}".format(repr(self.message))


def execute_api(command: str, on_failure_event: EventType,  max_attempts: int = 5, retry_delay: float = 1.0) -> typing.Callable:
                ...
                while attempts < max_attempts:
                    ...
                    try:
                        ...
                        if attempts == max_attempts:
                            raise NodeApi_Exception(command, on_failure_event, attempts, err)
